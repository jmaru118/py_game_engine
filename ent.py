...
class Player(Entity):
    ...
    def kill(self):
        self.engine.soundMgr.playPlayerDeath()
        if self.id == 0:
            self.node.translate(9000,0,-9000)
            self.pos = self.pos + MyVector(9000,0,-9000)
            self.engine.gfxMgr.camYawNode1.translate(9000,0,-9000)
        if self.id == 1:
            self.node.translate(-9000,0,9000)
            self.pos = self.pos + MyVector(-9000,0,9000)
            self.engine.gfxMgr.camYawNode2.translate(-9000,0,9000)
        t = threading.Timer(3.0,self.respawn)
        t.start()
    def respawn(self):
        random.seed()
        spawnChooser = random.randint(0,3)

        if spawnChooser == 0:
            spawnPoint = MyVector(-2000, 0, -2500)
        elif spawnChooser == 1:
            spawnPoint = MyVector(2000, 0, 2500)
        elif spawnChooser == 2:
            spawnPoint = MyVector(1400, 0, 0)
        elif spawnChooser == 3:
            spawnPoint = MyVector(-1400, 0, 0)
            
        if self.id == 0:

            self.node.translate(spawnPoint - self.pos)
            self.engine.gfxMgr.camYawNode1.translate(spawnPoint - self.pos)
            self.pos = spawnPoint
        elif self.id == 1:

            self.node.translate(spawnPoint - self.pos)
            self.engine.gfxMgr.camYawNode2.translate(spawnPoint - self.pos)
            self.pos = spawnPoint
       # elif (self.id == 1):
...
class Enemy(Entity):
	...
	self.engine.soundMgr.playEnemyDeath()
        print "kill"
        ...
        t = threading.Timer(10.0,self.respawn)
        t.start()


        
    def respawn(self):
        spawnPoint = self.engine.entityMgr.enemyPositions[self.index]
        self.node.translate(spawnPoint - self.pos)
        self.pos = spawnPoint
